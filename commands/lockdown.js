...
exports.run = function(bot, msg, args) {
  ...
  if(!action) return embed(0xff0000, "Invalid Lockdown Choice", "Please choose either `on` or `off` with your lockdown command!");
  let time = args.slice(1).join(" ");
  if(action.includes("on")) {
    if(!time) return embed(0xff0000, "Invalid input", "Please specify a time to lockdown the channel for!");
...
exports.help = {
  name: 'lockdown',
  description: 'Removes all permissions to type',
  usage: 'lockdown <time>'
};